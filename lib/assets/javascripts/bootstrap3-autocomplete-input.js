...
function autocomplete_query(obj){
    ...
    obj.typeahead({
        source: function (q, process) {
            ...
            $.getJSON( obj.attr('data-source-query'), {q: q},
                function( data ) {
                    ...
                    autocomplete_data_server = {};
                    autocomplete_data_server[name] = {"map":[], "values": []};

                    $.each(data, function (i, row) {
                        autocomplete_data_server[name]["values"].push(row[1]);
                        autocomplete_data_server[name]["map"][row[1]]=row[0];
                    });
                    process(autocomplete_data_server[name]["values"]);
                }
            );// get json
        }//source
        , updater: function (item) {
            ...
            if (obj_id) {
                var name = obj.attr('name');
                obj_id.val(autocomplete_data_server[name]["map"][item]);
            }
